...
import { getVolunteers, deleteVolunteer, searchVolunteers } from '../redux/features/volSlice';
...
const DonorDashboard = ({ title, firstName, lastName, nic, _id, email, des, phone, imgFile }) => {
    ...
    function generatePDF(vItems) {
        ...
        const tableColumn = ["Title", "First Name", "Last Name", "Email Address","NIC","Phone Number", "Description"];

        const tableRows = [];



        vItems.slice(0).reverse().map(vItem => {
            ...
            <div className="col-12 col-md-12 col-sm-12 col-12" style={{ marginTop: "100px" }}>
                <div className="row">
                    <div className="col-2 col-md-2 col-sm-2 col-2">
                        <button type="button" className="btn btn-outline-info" id="pdfButton" onClick={(e) => { generatePDF(volunteers) }}><i className="fa fa-file-pdf"></i>&nbsp;&nbsp;Download Report</button>
                    </div>
                    <div className="col-2 col-md-2 col-sm-2 col-2">
                        <MDBBtn color="success" href={"/add_vol"} style={{ float: 'left' }}><i className="fas fa-plus"></i>&nbsp; Create Volunteer</MDBBtn>
                    </div>
                            ...
                            {volunteers && volunteers.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td>
                                        ...
                                        &nbsp;
                                        <MDBBtn color="info" href={`/volunteer/${item._id}`}><i className="fas fa-eye"></i>&nbsp;</MDBBtn>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
