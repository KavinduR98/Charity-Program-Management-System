...
export const deleteBeneficiary = async (req, res) => {
    ...
    try {
      if (!mongoose.Types.ObjectId.isValid(id)) {
        return res.status(404).json({ message: `No beneficiary exist with id: ${id}` });
      }
  ...
  export const updateBeneficiary = async(req,res)=>{
    const {id} = req.params;
    const {firstName, lastName, address, email, phone, date, details} = req.body;
    ...
    try{
        ...
        await BeneficiaryModel.findByIdAndUpdate(id, updatedBeneficiary, {new:true});
...
export const getBeneficiariesBySearch = async(req,res)=>{
    const {searchQuery}=req.query;
    try{
        const firstName = new RegExp(searchQuery,"i");
        const beneficiaries = await BeneficiaryModel.find({firstName});
